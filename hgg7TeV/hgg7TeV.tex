...
For this reason, two data-driven background modelling tecnhiques were developed, one in which a fully parametric
description of the background from data is devised and one in which a binned model is constructed from 
sidebands in the $\mgg$ spectrum. The latter of these two serves as an independant cross-check of the former
...
Figure~\ref{fig:higgsrightwrongvtx} shows the invariant mass distributions from a SM
Higgs for events in which the vertex selected is within 10mm of the generated vertex
...
A BDT was trained to rank the standard collection of reconstructed verticies.
...
repsectively. In addition, the sum of the square of the transverse momenta of all the tracks associated 
to a given vertex is included to preferentially select hard interactions. If at least one of the 
photons converts to an $e^{+}e^{-}$ pair, the difference between the position in $z$ as calcuated 
using the electron-positron pair and that from the standard vertex, relative to the resolution in $z$
is included as an input variable. The BDT was traind on $\Hgg$ MC with a mass of 120 GeV. 
...
as a function of the $\pt$ of the $Z$ boson. This is used to correct the Higgs signal MC for 
...
calculate the probability in a given event that the correct vertex is assigned. The red line in Figure~\ref{}
shows a comparison of the per-event vertex probability estimated from the second BDT agaisnt the 
...
Where more than one diphoton pair satifies these criteria in an event, the pair which has the largest
...
the specific kinematics and toplogy of the event are exploited.
...
the input variables to the BDT are chosen to be mass-independant. In addition, this philosophy allows
for a fully data-driven estimation of the background shape as described in Section~\ref{sec:backgrounmodel}.
These input variables which describe the kinematics are:
...
Events below this cut value were found to provide negligible improvement in the expected limit.
...
Figure~\ref{fig:massmcdata} is the invariant mass distribution in data and MC for events passing the full
selection. The expected peak from a Higgs with mass 125 GeV, scaled by 10, is shown in red.
After the application of the full selection, the total background contains 76\% prompt diphoton events.
...
of the photons allowing for validation of the BDT's response to real photons from a resonance decay. 
Figure~\ref{} shows the diphoton BDT distribution in $\Zee$ MC and data after applying
...
Both the photon ID and regression BDT's rely on a correct simulation of the shower simulation 
...
in a high $\pt$ photons as descibed in Sections~\ref{sec:energyreconstruction} and~\ref{sec:phoidmva}. 
...
photon ID MVA (BDT output) in $\Zee$ MC and data treating the electrons as photons. The red lines
show the $\pm 1\sigma$ error envelopeattributed to the systematic uncertainty on the shower simulation.
...
The contribution to Higgs production from vector boson fusion ($qqH$) is around a factor ten smaller than that
of gluon-gluon fusion. However, additional information from the two jets associated 
with the $qqH$ production allows for further reduction of the diphoton background.
Events which pass the full selection and in addition
satisfy a series of criteria designed to target the specific topoloy of the dijet system are 
tagged as likely to have originated from $qqH$ production. Figure~\ref{fig:vbfdeta} shows the 
